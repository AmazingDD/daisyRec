...
@LastEditors: Yudi
@LastEditTime: 2019-12-17 17:55:13
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=40, help="number of epochs") # 100
    ...
    param_list = []
    for p1 in range(1, 6): # window
        for p2 in [10, 20, 50, 100, 150, 200]: # e_dim
            for p3 in range(1, 11): # n_negs
    ...
    f = open(tune_log_path + f'item2vec_{args.dataset}_{args.prepro}_{args.val_method}.csv', 
             'w', 
             encoding='utf-8')
    f.write('Pre,Rec,HR,MAP,MRR,NDCG,window,e_dim,n_negs' + '\n')
    ...
    for params in param_list:
        p_window, e_dim, n_negs = params
        print(f'Parameter Settings: window:{p_window}, e_dim:{e_dim}, n_negs:{n_negs}')
        ...
        for fold in range(fn):
            ...
            embed_model = Item2Vec(item_num=vocab_size, embedding_size=e_dim)
            model = SGNS(embedding=embed_model, item_num=vocab_size, n_negs=n_negs, weights=weights)
